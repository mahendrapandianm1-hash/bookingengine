...
export const Navbar = () => {
  ...
  const navLinks = [{name:'Home',path:'/'},{name:'About',path:'/about'},{name:'Rooms',path:'/rooms'}
  ...
  return(
        ...
        <nav  className={`fixed top-0 left-0 bg-gray-900/2 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4":"py-4 md:py-6"}`}>
            {/*Logo */}
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <button onClick={()=>navigation('/login')} className={`px-8 py-2.5 rounded-full ml-4 transition-all duration-500 ${isScrolled ? "invert":""} bg-black text-white`}>
            Login
        </button>
      </div>
      {/* Mobile Menu Button */}
...
<button onClick={()=>navigation('/login')} className="bg-black text-white px-8 py-2.5 rounded-full transition-all duration-500">
                        Login
                    </button>
      </div>
        </nav>
